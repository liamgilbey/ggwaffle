...
devtools::install_github("ggwaffle")
```

## Usage
ggwaffle heavily relies on the usage of [ggplot2](https://github.com/tidyverse/ggplot2). Much like standard graphs, waffle charts are created by adding layers to a base graphic.
...
`aes_d` is obviously coined from ggplot's `aes` function and has a very similar idea. Here we are mapping column names to feed into a function so they can be renamed for use in the function.
```r
library(ggwaffle)
...
Functions have also been included to make the default visualisations more visually appealling.
